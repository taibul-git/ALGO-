...
router.get('/', requireAuth, async (req, res) => {
  ...
  const rows = await dbAll(`
    SELECT i.*, u.full_name as assigned_agent_name,
      (SELECT n.note_text FROM notes n WHERE n.entity_type = 'issue' AND n.entity_id = i.id ORDER BY n.created_at DESC LIMIT 1) as latest_note
    FROM issues i
