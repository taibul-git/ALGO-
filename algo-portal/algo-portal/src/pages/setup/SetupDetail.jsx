...
export default function SetupDetail() {
  ...
  if (!setup) return <div className="text-[var(--text-faint)] text-sm">Loading…</div>;

  return (
    <div>
      <Link to="/setups" className="inline-flex items-center gap-1 text-sm text-[var(--text-muted)] hover:text-[var(--text-secondary)] mb-4">
        <ArrowLeft size={15} /> Back to setups
      </Link>

      <div className="flex items-start justify-between mb-6">
        <div>
          <h1 className="text-2xl font-semibold text-[var(--text-primary)]">{setup.telegram_name}</h1>
          <div className="flex items-center gap-2 mt-1">
            <StatusBadge status={setup.setup_status} />
            <span className="text-sm text-[var(--text-faint)] capitalize">{setup.plan_type.replace('_', ' ')} plan</span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-3 gap-6">
        <div className="col-span-2 space-y-6">
          <div className="bg-[var(--bg-panel)] rounded-xl border border-[var(--border)] p-5">
            <h3 className="font-medium text-[var(--text-primary)] mb-1">Account information (read-only — owned by CS Portal)</h3>
            <p className="text-xs text-[var(--text-faint)] mb-4">These fields are synced from the client record and can't be edited from Setup.</p>
            <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
              <div><span className="text-[var(--text-faint)]">Account #:</span> <span className="text-[var(--text-secondary)]">{setup.trading_account_number || '—'}</span></div>
              <div><span className="text-[var(--text-faint)]">Platform:</span> <span className="text-[var(--text-secondary)]">{setup.trading_platform || '—'}</span></div>
              <div><span className="text-[var(--text-faint)]">Server:</span> <span className="text-[var(--text-secondary)]">{setup.server_name || '—'}</span></div>
              <div><span className="text-[var(--text-faint)]">Algo Plan:</span> <span className="text-[var(--text-secondary)]">{setup.algo_plan || '—'}</span></div>
              <div><span className="text-[var(--text-faint)]">Fixed Lot:</span> <span className="text-[var(--text-secondary)]">{setup.fixed_lot_size || '—'}</span></div>
              <div><span className="text-[var(--text-faint)]">Account Balance:</span> <span className="text-[var(--text-secondary)]">{setup.account_balance || '—'}</span></div>
            </div>
          </div>

          <div className="bg-[var(--bg-panel)] rounded-xl border border-[var(--border)] p-5">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-medium text-[var(--text-primary)]">Setup fields</h3>
            ...
            <p className="text-xs text-[var(--text-faint)] mt-1">Source: {setup.source_tab}</p>
          </div>

          <div className="bg-[var(--bg-panel)] rounded-xl border border-[var(--border)] p-5">
            <h3 className="font-medium text-[var(--text-primary)] mb-1">Client issues</h3>
            <p className="text-xs text-[var(--text-faint)] mb-4">Marking Resolved/Pending here updates instantly in the CS Portal too — same record, both places.</p>
            <div className="space-y-3">
              ...
              {!(setup.issues || []).length && <p className="text-sm text-[var(--text-faint)]">No issues logged for this client.</p>}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="bg-[var(--bg-panel)] rounded-xl border border-[var(--border)] p-5">
            ...
            <div className="space-y-3 max-h-64 overflow-y-auto">
              ...
              {!setup.notes.length && <p className="text-sm text-[var(--text-faint)]">No notes yet.</p>}
            </div>
          </div>

          <div className="bg-[var(--bg-panel)] rounded-xl border border-[var(--border)] p-5">
            <h3 className="font-medium text-[var(--text-primary)] mb-3">Activity</h3>
            <div className="space-y-2 max-h-64 overflow-y-auto">
              {setup.activity.map((a) => (
                <div key={a.id} className="text-xs text-[var(--text-muted)]">
                  <span className="text-[var(--text-primary)]">{a.user_name || 'System'}</span> {a.action.replace('_', ' ')} {a.field_changed && `· ${a.field_changed}`}
                  <div className="text-[var(--text-faint)]">{a.created_at}</div>
                </div>
              ))}
              {!setup.activity.length && <p className="text-sm text-[var(--text-faint)]">No activity yet.</p>}
            </div>
          </div>
        </div>
      </div>
