...
export default function SetupsList() {
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-[var(--text-primary)]">Setups</h1>
        <p className="text-[var(--text-muted)] text-sm mt-1">Auto-synced from the CS Portal — update setup-specific fields here without touching customer identity data.</p>
      </div>
      ...
      <div className="bg-[var(--bg-panel)] rounded-xl border border-[var(--border)]">
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {data.rows.map((s) => (
              <tr key={s.id} className="border-b border-[var(--border)] hover:bg-[var(--bg-panel-hover)]">
                <td className="px-4 py-3 font-medium text-[var(--text-primary)]">{s.telegram_name}</td>
                <td className="px-4 py-3 text-[var(--text-muted)] capitalize">{s.plan_type.replace('_', ' ')}</td>
                <td className="px-4 py-3 text-[var(--text-muted)]">{s.vps_no || '—'}</td>
                <td className="px-4 py-3 text-[var(--text-muted)]">{s.activation_date || '—'}</td>
                <td className="px-4 py-3 text-[var(--text-muted)]">{s.expire_date || s.vps_expire_date || '—'}</td>
                <td className="px-4 py-3"><StatusBadge status={s.setup_status} /></td>
                <td className="px-4 py-3 text-xs text-[var(--text-faint)] max-w-[160px] truncate" title={s.source_tab}>{s.source_tab}</td>
                <td className="px-4 py-3 text-right"><Link to={`/setups/${s.id}`} className="text-[#3ECF8E] hover:underline font-medium">View</Link></td>
              </tr>
            ))}
            {!data.rows.length && <tr><td colSpan={8} className="px-4 py-10 text-center text-[var(--text-faint)]">No setups found.</td></tr>}
          </tbody>
        </table>
