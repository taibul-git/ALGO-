    ...
    <div>
          ...
          <tbody>
            {data.rows.map((c) => (
              <tr key={c.id} className="border-b border-slate-50 hover:bg-slate-50">
                ...
                <td className="px-4 py-3 text-slate-600">{c.assigned_agent_name || <span className="text-slate-300">Unassigned</span>}</td>
            ...
            {!data.rows.length && (
              <tr><td colSpan={7} className="px-4 py-10 text-center text-slate-400">No clients found.</td></tr>
            )}
          </tbody>
        </table>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Add new client" wide>
        <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-x-4">
          ...
          <Field label="Account Type"><input className={inputCls} value={form.account_type} onChange={(e) => setForm({ ...form, account_type: e.target.value })} placeholder="Personal / Prop Firm" /></Field>
          ...
          <Field label="Algo Plan"><input className={inputCls} value={form.algo_plan} onChange={(e) => setForm({ ...form, algo_plan: e.target.value })} placeholder="Algo Lifetime / One Month / Broker Trial" /></Field>
